...
def _mfa_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(settings, "auth_mfa_encryption_keys", {})
...
def test_keyring_decrypts_previous_version_during_rotation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    first_key = settings.auth_mfa_encryption_key
    secret = mfa.generate_totp_secret()
    encrypted_v1 = mfa.encrypt_totp_secret(secret, user_id="user-1")
    second_key = base64.urlsafe_b64encode(secrets.token_bytes(32)).decode("ascii")
    monkeypatch.setattr(settings, "auth_mfa_key_version", 2)
    monkeypatch.setattr(settings, "auth_mfa_encryption_key", second_key)
    monkeypatch.setattr(
        settings,
    ...
    assert mfa.decrypt_totp_secret(encrypted_v1, user_id="user-1") == secret
    assert mfa.encrypt_totp_secret(secret, user_id="user-1").key_version == 2
